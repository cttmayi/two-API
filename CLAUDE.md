...
- 代码改动和版本号默认分开提交：功能/修复提交只包含实际代码变化，版本号提交使用类似 `chore: bump version to x.y.z` 的独立提交；除非用户特别要求合并。
- 版本号使用 `主版本.功能版本.补丁版本` 三段格式：bug 修复和小优化递增补丁版本（`0.0.1`）；向后兼容的新功能、配置项、端点或 dashboard 能力递增功能版本并重置补丁版本（`0.1.0`）；破坏兼容性的配置、接口、行为变更递增主版本并重置后两位（`1.0.0`）。文档-only 改动默认不升级版本，除非用户要求发布。
